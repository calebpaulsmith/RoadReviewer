...
try {
  ...
  # Column indices: Class 19, Urban/Rural 20, ACUB Name 21, Road Name 22,
  # Street Name 23 (new TIGER), Federal Aid Status 24.
  ...
  $sites.Cells(2, 24).Value2 = 'Failed - simulated test failure'   # Federal Aid Status (col 24 = X)
  ...
  $sites.Cells(3, 24).Value2 = 'Non-federal aid - Urban Local (sticky)'   # Federal Aid Status

  Write-Host "  Before re-run:"
  Write-Host ("    row 2 elig = '" + [string]$sites.Cells(2,24).Value2 + "'")
  Write-Host ("    row 3 class = '" + [string]$sites.Cells(3,19).Value2 + "'  elig = '" + [string]$sites.Cells(3,24).Value2 + "'")

  $excel.Run('ReRunFailedClassifications') | Out-Null

  Write-Host "  After re-run:"
  $r2 = [string]$sites.Cells(2,24).Value2
  $r3_class = [string]$sites.Cells(3,19).Value2
  $r3_elig = [string]$sites.Cells(3,24).Value2
